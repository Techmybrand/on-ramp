...
import { SendReturnResult, SendReturn, Send, SendOld, __DEV__ } from './types'
...
export class NoBitkeepProviderError extends Error {
  public constructor() {
    ...
    this.message = 'No Bitkeep Ethereum provider was found on window.bitkeep.ethereum.'
  }
}
...
export class BitkeepConnector extends AbstractConnector {
  ...
  public async getAccount(): Promise<null | string> {
    ...
    let account
    try {
      account = await (window.bitkeep.ethereum.send as Send)('eth_accounts').then(sendReturn => parseSendReturn(sendReturn)[0])
    } catch {
